...
<body>
	...
	<div style="width:95%;margin:auto;">
		...
		<table class="STHSWaiver_Table">
			...
			<tbody>
				<?php
				if (empty($Waiver) == false) {
					while ($Row = $Waiver->fetchArray()) {
						if ($Row['Player'] > 10000) {
							echo "<tr><td><a href=\"GoalieReport.php?Goalie=" . ($Row['Player'] - 10000) . "\"</a>" . substr($Row['PlayerNameOV'], 0, -5) . "</td>";
						} else {
							echo "<tr><td><a href=\"PlayerReport.php?Player=" . $Row['Player'] . "\"</a>" . substr($Row['PlayerNameOV'], 0, -5) . "</td>";
							echo "<td>";
							echo "<img src=\"./images/Pro-team/" . $Row['FromTeam'] . ".png\" alt=\"\" class=\"STHSPHPTeamStatsTeamImage\" />";
						}
						echo $Row['FromTeamName'] . "</td>";
						echo "<td>";
						if ($Row['ToTeam'] != 0) {
							echo "<img src=\"./images/pro-team/" . $Row['ToTeam'] . ".png\" alt=\"\" class=\"STHSPHPTeamStatsTeamImage\" />";
						}
		...
		<?php
		if (empty($WaiverOrder) == false) {
			while ($Row = $WaiverOrder->fetchArray()) {
