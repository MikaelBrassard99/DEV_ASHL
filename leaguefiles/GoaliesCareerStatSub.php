...
<tbody>
	...
	foreach ($arrayDbs as $Dbs) {
		if ($dictGoaliesProSeasonStats[$Order] <> Null) {
			if ($dictGoaliesProSeasonStats[$Order]['GP'] <> 0 && $dictGoaliesProSeasonStats[$Order]['GP'] <> Null) {
				$Row = $dictGoaliesProSeasonStats[$Order];
			...
			if ($dictGoaliesProStats[$Order] <> Null) {
				if ($dictGoaliesProStats[$Order]['GP'] <> 0 && $dictGoaliesProStats[$Order]['GP'] <> Null) {
					$Row = $dictGoaliesProStats[$Order];
					$Row1 = $dictGoalieInfo[$Order];
					$Order += 1;
					echo "<tr><td>" . $Order . "</td>";
					echo "<td><a>" . getSeasonName($Dbs) . "</a></td>";
