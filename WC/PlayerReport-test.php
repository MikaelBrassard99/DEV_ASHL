...
<body>
	...
	<div class="STHSPHPPlayerStat_PlayerNameHeader">
		...
		echo "<tr><td><br /></td></tr>
		<tr>
		<td style=\"padding-bottom: 10px; text-align: center;display: flex;justify-content: center;align-items: center;\" >" . $PlayerName . " #" . $PlayerInfo['Jersey'] . "&nbsp" . "<img src=\"https://www.countryflags.io/" . getCountryAbre(substr($PlayerInfo['Country'], 0, -1)) . "/shiny/24.png\"></td>
		</tr>
		...
		if ($PlayerInfo <> Null) {
			if ($PlayerInfo['Retire'] == 'False') {
				echo "<div id=\"cssmenu\" style=\"display:inline-block\">
						<ul style=\"max-width:150px;width:100%;margin:0 auto\">
							<li style=\"font-size:24px;cursor:pointer;line-height:0\">&#9660;
						<ul style=\"max-height:250px;overflow-x:hidden;overflow-y:scroll\">";
		...
		<div class="tabsmain standard">
			...
			<div class="STHSPHPPlayerStat_Tabmain-content">
				...
				<div class="tabmain" id="tabmain2">
					...
					<table class="tablesorter STHSPHPProPlayerStatPerTeam_Table">
						<thead>
							...
							<?php
							if ($Team >= 0) {
								echo "<th class=\"columnSelector-false STHSW55\" data-priority=\"6\" title=\"Team Name\">" . $GeneralStatLang['Team'] . "</th>";
							} else {
								echo "<th data-priority=\"2\" title=\"Team Name\" class=\"STHSW55\">" . $GeneralStatLang['Name'] . "</th>";
							}
							?>
					...
					<table class="tablesorter STHSPHPProPlayerStatPerTeam_Table">
						<thead>
							...
							if ($Team >= 0) {
								...
								echo "<th data-priority=\"2\" title=\"Team Name\" class=\"STHSW55\">" . $GeneralStatLang['Team'] . "</th>";
							}
							?>
							<th data-priority="1" title="Games Played" class="STHSW25">GP</th>
							<th data-priority="1" title="Goals" class="STHSW25">G</th>
				...
				<div class="tabmain" id="tabmain3">
					...
					<table class="tablesorter STHSPHPProPlayerStatPerTeam_Table">
						...
						<?php foreach ($arrayDbs as $Dbs) {
							$count = 0;

							//$db = new SQLite3($Dbs);

							$Query = "SELECT PlayerProStat.*, PlayerInfo.Team, PlayerInfo.TeamName,PlayerInfo.PosC, PlayerInfo.PosLW, PlayerInfo.PosRW, PlayerInfo.PosD, ROUND((CAST(PlayerProStat.G AS REAL) / (PlayerProStat.Shots))*100,2) AS ShotsPCT, ROUND((CAST(PlayerProStat.SecondPlay AS REAL) / 60 / (PlayerProStat.GP)),2) AS AMG,ROUND((CAST(PlayerProStat.FaceOffWon AS REAL) / (PlayerProStat.FaceOffTotal))*100,2) as FaceoffPCT,ROUND((CAST(PlayerProStat.P AS REAL) / (PlayerProStat.SecondPlay) * 60 * 20),2) AS P20, 0 as Star1, 0 as Star2, 0 As Star3 FROM PlayerInfo INNER JOIN PlayerProStat ON PlayerInfo.Number = PlayerProStat.Number WHERE PlayerProStat.Number = " . $Player;
							$PlayerProStatAdvanced = $db->query($Query);

							$Query = "SELECT PlayerProSeasonStat.*, PlayerInfo.Team, PlayerInfo.TeamName,PlayerInfo.PosC, PlayerInfo.PosLW, PlayerInfo.PosRW, PlayerInfo.PosD, ROUND((CAST(PlayerProSeasonStat.G AS REAL) / (PlayerProSeasonStat.Shots))*100,2) AS ShotsPCT, ROUND((CAST(PlayerProSeasonStat.SecondPlay AS REAL) / 60 / (PlayerProSeasonStat.GP)),2) AS AMG,ROUND((CAST(PlayerProSeasonStat.FaceOffWon AS REAL) / (PlayerProSeasonStat.FaceOffTotal))*100,2) as FaceoffPCT,ROUND((CAST(PlayerProSeasonStat.P AS REAL) / (PlayerProSeasonStat.SecondPlay) * 60 * 20),2) AS P20, 0 as Star1, 0 as Star2, 0 As Star3 FROM PlayerInfo INNER JOIN PlayerProSeasonStat ON PlayerInfo.Number = PlayerProSeasonStat.Number WHERE PlayerProSeasonStat.Number = " . $Player;
							$PlayerSeasonStatAdvanced = $db->query($Query);
				...
				<div class="tabmain" id="tabmain4">
					<br />
					<div class="STHSPHPPlayerStat_TabHeader"><?php echo $PlayersLang['FarmStat'] . ': Saison régulière | Séries'; ?></div><br />
					<table class="tablesorter STHSPHPProPlayerStatPerTeam_Table">
						<tr>
							<th><?php echo $GeneralStatLang['Season']; ?></th>
							<th><?php echo $GeneralStatLang['Team']; ?></th>
							...
							<th style="border-left: 2px solid red;"><?php echo $GeneralStatLang['GamePlayed']; ?></th>
							<th><?php echo $GeneralStatLang['Goals']; ?></th>
							<th><?php echo $GeneralStatLang['Assists']; ?></th>
							<th><?php echo $GeneralStatLang['Points']; ?></th>
							<th><?php echo $GeneralStatLang['PlusMinus']; ?></th>
						...
						foreach ($arrayDbs as $Dbs) {
							echo "<tr>";
							echo "<td>";
							if ($dictPlayerFarmStats[$count] <> Null) {
								echo $dictLeagueGeneral[$count]['LeagueYear'];
							} else {
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmStats[$count] <> Null) {
								echo $dictPlayerInfo[$count]['TeamName'];
							} else {
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count] <> Null) {
								echo $dictPlayerFarmSeasonStats[$count]['GP'];
							} else {
								echo $dictPlayerFarmStats[$count]['GP'];
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count]  <> Null) {
								echo $dictPlayerFarmSeasonStats[$count]['G'];
							} else {
								echo $dictPlayerFarmStats[$count]['G'];
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count]  <> Null) {
								echo $dictPlayerFarmSeasonStats[$count]['A'];
							} else {
								echo $dictPlayerFarmStats[$count]['A'];
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count]  <> Null) {
								echo $dictPlayerFarmSeasonStats[$count]['P'];
							} else {
								echo $dictPlayerFarmStats[$count]['P'];
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count]  <> Null) {
								echo $dictPlayerFarmSeasonStats[$count]['PlusMinus'];
							} else {
								echo $dictPlayerFarmStats[$count]['PlusMinus'];
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count]  <> Null) {
								echo $dictPlayerFarmSeasonStats[$count]['Pim'];
							} else {
								echo $dictPlayerFarmStats[$count]['Pim'];
							}
							echo "</td>";
							echo "<td>";
							if ($dictPlayerFarmSeasonStats[$count]  <> Null) {
								echo number_format($dictPlayerFarmSeasonStats[$count]['AMG'], 2);
							} else {
								if ($dictPlayerFarmSeasonStats[$count]['AMG'] == Null && $dictPlayerFarmSeasonStats[$count]['AMG'] == Null) {
									echo "";
								} else {
									echo number_format($dictPlayerFarmStats[$count]['AMG'], 2);
								}
							}
							echo "</td>";
