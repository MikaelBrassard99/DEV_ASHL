...
<tbody>
	...
	foreach ($arrayDbs as $Dbs) {
		if ($dictGoaliesFarmSeasonStats[$Order] <> Null) {
			if ($dictGoaliesFarmSeasonStats[$Order]['GP'] <> 0 && $dictGoaliesFarmSeasonStats[$Order]['GP'] <> Null) {
				$Row = $dictGoaliesFarmSeasonStats[$Order];
			...
			if ($dictGoaliesFarmStats[$Order] <> Null) {
