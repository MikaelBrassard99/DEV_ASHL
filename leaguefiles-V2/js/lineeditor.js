...
function onClickEventPlayer(column, value) {
	var input = document.getElementById('playerSelectF');
	input.innerHTML = ''
	var splitColumn = column.split("|");
	var splitValues = value.split("|");
	var containerTr1 = document.createElement('tr');
	input.appendChild(containerTr1);

	splitColumn.forEach(function (item) {
		// Create a new paragraph element
		var columnElements = document.createElement('th');

		// Set the text content of the paragraph element to the array element
		columnElements.textContent = item;

		input.appendChild(columnElements);
	});

	var containerTr2 = document.createElement('tr');
	input.appendChild(containerTr2);

	splitValues.forEach(function (item) {
		// Create a new paragraph element
		var valueElements = document.createElement('td');

		// Set the text content of the paragraph element to the array element
		valueElements.textContent = item;

		// Append the paragraph element to the container
		input.appendChild(valueElements);
	});
}
