...
<th class="sorter-false" colspan="12"><?php echo $TeamStatLang['Overall']; ?></th>
...
<th class="sorter-false" colspan="42"></th>
</tr>
<tr>
	<th data-priority="3" title="Order Number" class="STHSW10 sorter-false">#</th>
	<th data-priority="critical" title="Team Name" class="STHSW200"><?php if ($Team <> 0) {
																		echo "VS ";
																	} ?><?php echo $TeamStatLang['TeamName']; ?></th>
	<?php if ($CareerLeaderSubPrintOut == 1) {
		echo "<th data-priority=\"2\" title=\"Year\" class=\"STHSW25\">Year</th>";
	} ?>
...
<tbody>
	...
	$Order = 0;
	$NoSort = (bool)FALSE;
	if (empty($TeamStatSub) == false) {
		while ($row = $TeamStatSub->fetchArray()) {
			$Order += 1;
			if ($row['Number'] <= 100) {
				echo "<tr><td>" . $Order . "</td>";
			...
			if ($row['GP'] > 0 and $LeagueGeneral['PointSystemW'] > 0) {
				echo "<td>" . number_Format(($row['Points'] / ($row['GP'] * $LeagueGeneral['PointSystemW'])), 3) . "</td>";
			} else {
